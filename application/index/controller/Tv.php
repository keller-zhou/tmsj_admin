...
class tv extends SuncommonRyan
{
	...
	public function advert()
	{
		...
		$_var_1 = session('code');
		return view('advert', ['code' => $_var_1]);
	}
	public function index()
	{
		$_var_2 = input('code');
		$_var_3 = input('msg');
		$_var_4 = db('tv')->order('id desc')->paginate(12);
	...
	{
		$_var_6 = input('msg');
		$_var_7 = db('tv')->where('id', input('id'))->find();
		return view('update', ['data' => $_var_7, 'code' => $_var_6]);
	}
	public function del()
	{
		$_var_8 = db('tv')->where('id', input('id'))->delete();
		...
		if ($_var_9) {
			...
			if ($_var_10) {
				...
				if (in_array($_var_12, $_var_11)) {
					...
					$_var_14['content'] = input('content');
					$_var_14['type'] = input('type');
					if (db('tv')->insert($_var_14) !== false) {
				...
				return redirect('tv/add', ['code' => 0, 'msg' => '上传失败' . $_var_9->getError()]);
			}
		} else {
	...
	public function edit()
	{
		...
		if ($_var_15) {
			...
			if ($_var_16) {
				...
				return redirect('tv/add', ['code' => 0, 'msg' => '上传失败' . $_var_15->getError()]);
			}
		}
		$_var_20['title'] = input('title');
		$_var_20['url'] = input('url');
		$_var_20['content'] = input('content');
		$_var_20['type'] = input('type');
		if (db('tv')->where('id', input('id'))->update($_var_20) !== false) {
			return redirect('tv/index', ['code' => 1, 'msg' => '添加成功']);
		} else {
			unlink($_var_19);
			return redirect('tv/add', ['code' => 0, 'msg' => '添加失败']);
